...
|  Traversing through BST   |       O(n)       |    traverse_forwards and traverse_reverse        |
...
As you can see every functionality to a BST is O(n) or faster. For example, lets say we are looking for the node containing the number 5 and the root node contains the number 1. Since we know 5 is greater than 1 we don't even have to search through the left side of the BST. See how fast that is? With just the first comparison we can cut the search by half immediately.
...
# Problem to solve
With this problem I want to give you an option to fix some mistakes as well as writing out your own functions for a BST. There are going to be 4 things to do for this problem which consist of: 
